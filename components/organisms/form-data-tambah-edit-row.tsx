      ...
      <form onSubmit={handleSubmit(handleSubmitForm)}>
          ...
          <div className="grid grid-cols-1 gap-4 mb-4">
            {column.map((item: any, index: number) => (
              <div key={index} className="col-span-1">
                ...
                <input {...register(item.name)} type="" id="author" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-emerald-500 focus:border-emerald-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-emerald-500 dark:focus:border-emerald-500" placeholder={`Masukkan ${item.name}`} />
              </div>
            )
            )}
          </div>
        ...
        <ModalFooter>
          <div className="flex justify-between w-full">
            ...
            <button type="submit" className="text-white bg-emerald-600 hover:bg-emerald-800 focus:ring-4 focus:outline-none focus:ring-emerald-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-emerald-600 dark:hover:bg-emerald-700 dark:focus:ring-emerald-800">Tambahkan ke tabel</button>
          </div>
        </ModalFooter>
      </form>
    </>
